...
from .forms import PropostaCriacaoForm, PropostaItemForm, PropostaLinhaPublicaForm, PropostaRevisaoForm, PropostaTributoForm
from .models import Proposta, PropostaRevisao
from .services import calcular_precificacao, criar_nova_revisao, criar_proposta, enviar_proposta, montar_contexto_publico_proposta
...
def proposta_detalhe(request, pk):
    proposta = get_object_or_404(Proposta.objects.select_related("empresa", "cliente"), pk=pk)
    revisao = _revisao_atual(proposta)
    try:
        calculo = calcular_precificacao(revisao)
    except ValidationError as erro:
        calculo = {"erro": erro.messages[0]}
    return render(request, "comercial/proposta_detalhe.html", {"proposta": proposta, "revisao": revisao, "calculo": calculo})
